...
if [ $COCKROACHDB_SECURE == False ]; then
     ./opt/unsecure.sh
fi
sleep $LOAD_WAIT
...
echo "#################################################################"
if [ $NODE_MASTER == False ] && [ ${node} -eq 1 ]; then
        sleep 3
        users_list=$(cat ${cert}/users.txt)
        amount=$(cat ${cert}/users.txt | wc -l )
        num=$(expr ${amount} - 1)
        ...
        while [ $counter -le $num ]; do
            ...
            echo "#################################################################"
            cockroach user set $username --certs-dir=$COCKROACH_CERTS_DIR --host=$COCKROACH_MASTERNODE_IP
        counter=$(expr $counter + 1)
        done
fi
...
if [ $NODE_ADDITION == True ] && [ $node -ge 1 ]; then
    ...
    cockroach start --certs-dir=${COCKROACH_CERTS_DIR} --host=$HOST_IPADDRESS --store=node-0$STORE_ID --join=$COCKROACH_MASTERNODE_IP:26257  --http-port=808$STORE_ID
    cockroach start --certs-dir=${COCKROACH_CERTS_DIR} --store=node-0$STORE_ID --host=$HOST_IPADDRESS --port=${port} --http-port=${http_port} --http-host=$ADVERTISE_HOST --join=$COCKROACH_MASTERNODE_IP:26257
    else
